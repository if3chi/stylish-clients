...
<div class="mt-1 sm:mt-0 sm:col-span-2">
    <div class="flex items-center">
        ...
        <div x-data="{ focused: false }">
            <input {{ $attributes }} @focus="focused = true" @blur="focused = false" type="file"
                class="sr-only" />

            <label for="{{ $for }}" :class="{ 'outline-none ring-2 ring-offset-2 ring-indigo-500': focused }"
                ...
                <x-icon.three-dots wire:loading wire:target="{{ $target }}" class="w-6 h-6 mr-2" />
                {{ $secLabel }}
            </label>
        </div>
    </div>
