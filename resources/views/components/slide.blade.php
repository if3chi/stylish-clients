...
<div x-data="{ open: @entangle($attributes->wire('model')).defer }" @keydown.window.escape="open = false" x-show="open" class="fixed inset-0 overflow-hidden"
    ...
    <div class="absolute inset-0 overflow-hidden">
        <div x-description="Background overlay, show/hide based on slide-over state."
            ...
            x-transition:leave-end="opacity-0" @click="open = true" aria-hidden="true">
        </div>
                ...
                <form class="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
                    <div class="flex-1">
                        ...
                        <div class="px-4 py-6 bg-gray-50 sm:px-6">
                            <div class="flex items-start justify-between space-x-3">
                                <div class="space-y-1">
                                    <h2 class="flex items-center text-lg font-medium text-indigo-800"
                                        id="slide-over-title">
                                        @if ($title === 'Edit')
                        ...
                        <div class="py-6 space-y-6 sm:py-0 sm:space-y-0 sm:divide-y sm:divide-gray-200">
                            ...
                            <!-- Birthday -->
                            <x-input.group>
                                <x-input.date wire:model.lazy="birthday" :error="$errors->first('birthday')" type="text"
                                    label="Client's Birthday" name="birthday" id="birthday" for="birthday"
                            ...
                            <!-- Client Type -->
                            <x-input.group>
                                <x-input.select wire:model.lazy="editing.type" label="Client Type" for="clientType"
                                    id="clientType" :error="$errors->first('editing.type')" />
                            </x-input.group>
                        </div>
                    </div>
                    ...
                    <div class="flex-shrink-0 px-4 border-t border-gray-200 py-5 sm:px-6">
                        <div class="space-x-3 flex justify-end">
                            <button type="button" wire:click.defer="$set('show_form', false)"
